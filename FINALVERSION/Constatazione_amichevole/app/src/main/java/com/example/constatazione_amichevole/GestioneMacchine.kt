    ...
    Card(
        ...
        Row(
            ...
            Column(modifier = Modifier.weight(1f)) {
                ...
                Text(
                    text = "Colore: ${vehicle.color}",
                    fontSize = 16.sp
                )
            }
            IconButton(onClick = { onEdit(vehicle) }) {
                Icon(Icons.Filled.Edit, contentDescription = "Modifica Macchina")
            }
            IconButton(onClick = { onDelete(vehicle) }) {
                Icon(Icons.Filled.Delete, contentDescription = "Delete Macchina")
            }
        }
    }
}
...
fun VehicleDialog(
    ...
    AlertDialog(
        ...
        title = { Text(text = if (vehicle == null) "Aggiungi Macchina" else "Modifica Macchina") },
        text = {
            Column {
                TextField(
                    value = name,
                    onValueChange = { name = it },
                    label = { Text("Modello della macchina") }
                )
                TextField(
                    value = year,
